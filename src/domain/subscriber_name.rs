...
impl SubscriberName {
    pub fn parse(s: impl Into<String>) -> Result<SubscriberName, SubscriberNameError> {
        let s: String = s.into();
...
impl From<SubscriberName> for String {
    fn from(subscriber_email: SubscriberName) -> Self {
...
mod tests {
    ...
    fn a_256_grapheme_name_is_valid() {
        let name = SubscriberName::parse("a̐".repeat(256));
        assert_ok!(name);
    }
    ...
    fn a_name_consisting_of_whitespace_is_rejected() {
        let name = SubscriberName::parse(" ".repeat(10));
        assert_err!(name);
    }
    ...
    fn a_name_containing_invalid_characters_is_rejected() {
        for ch in FORBIDDEN_CHARACTERS.iter() {
            let name = SubscriberName::parse(*ch);
            assert_err!(
                name,
